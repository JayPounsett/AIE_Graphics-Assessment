  ...
  unsigned int* indices)
{
  ...
  // Fill vertex buffer
  glBufferData(
    GL_ARRAY_BUFFER, vertexCount * sizeof(Vertex), vertices, GL_STATIC_DRAW);
...
void Mesh::InitialiseFromFile(const char* fileName)
{
  ...
  for (int i = 0; i < numV; i++) {
    vertices[i].position = glm::vec4(
      mesh->mVertices[i].x, mesh->mVertices[i].y, mesh->mVertices[i].z, 1);
    vertices[i].normal = glm::vec4(
      mesh->mNormals[i].x, mesh->mNormals[i].y, mesh->mNormals[i].z, 0);

    // UVs
    if (mesh->mTextureCoords[0]) {
      vertices[i].texCoord = glm::vec2(
        mesh->mTextureCoords[0][i].x, 1.0f - mesh->mTextureCoords[0][i].y);
    ...
    if (mesh->HasTangentsAndBitangents()) {
      vertices[i].tangent = glm::vec4(
        mesh->mTangents[i].x, mesh->mTangents[i].y, mesh->mTangents[i].z, 1);
    }
  }
...
void Mesh::LoadMaterial(const char* fileName)
{
  ...
  // rfind() searches in reverse, in this case starting at the end of the
  // filename.
  if (index != -1) {
  ...
  const std::vector<unsigned int>& indices)
{
  ...
  for (unsigned int a = 0; a < vertexCount; a++) {
    ...
    vertices[a].tangent.w =
      (glm::dot(glm::cross(glm::vec3(n), glm::vec3(t)), glm::vec3(tan2[a])) <
       0.0F)
        ? 1.0F
        : -1.0F;
  }
