...
namespace aie {
    ...
    class Shader {
      public:
        ...
        Shader(unsigned int stage, const char *filename): m_stage(0), m_handle(0), m_lastError(nullptr) { loadShader(stage, filename); }
        ~Shader();
    ...
    class ShaderProgram {
      public:
        ShaderProgram(): m_program(0), m_lastError(nullptr) { m_shaders[0] = m_shaders[1] = m_shaders[2] = m_shaders[3] = m_shaders[4] = 0; }
        ~ShaderProgram();
